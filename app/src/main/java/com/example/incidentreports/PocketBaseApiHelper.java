...
public class PocketBaseApiHelper {
    ...
    public void checkServerConnection(ConnectivityCallback callback) {
        String url = baseUrl + "/api/health";
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, url, null,
                response -> callback.onSuccess("Server reachable: " + baseUrl),
    ...
    private String parseVolleyError(VolleyError error) {
        if (error instanceof TimeoutError) {
            return "Connection timed out. Check PocketBase URL and network.";
        }
        if (error instanceof NoConnectionError || error instanceof NetworkError) {
            return "Unable to connect to server. Verify URL, Wi-Fi network, firewall, and PocketBase host binding (0.0.0.0).";
        }
