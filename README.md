...
### B. Base URL used by this version

This version uses `BuildConfig.POCKETBASE_URL` from Gradle and defaults to:

```kts
buildConfigField("String", "POCKETBASE_URL", "\"http://127.0.0.1:8090\"")
```

- `127.0.0.1` works when the app runs on the same machine/emulator loopback context.
- For physical devices, change this URL to your computer LAN IP (for example `http://192.168.1.20:8090`) and rebuild.

### C. Common unreachable-host fix

If your PocketBase terminal shows `started at http://127.0.0.1:8090` and your phone says unreachable,
use LAN bind and LAN URL:

1. Start with host bind:
   ```bash
   ./pocketbase serve --http=0.0.0.0:8090
   ```
2. Ensure phone + PC are on same Wi‑Fi.
3. Allow firewall inbound TCP 8090.
4. Open `http://<PC_LAN_IP>:8090/api/health` in phone browser.
